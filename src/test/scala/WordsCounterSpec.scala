...
import words_counter.{LineCounter, WordsCounter}
...
class WordsCounterSpec extends FunSpec {
  describe("line words counter") {
    ...
    it("should count words in the middle and add not finished marks when line wrapped without punctuation") {
      assert(WordsCounter.lineWordsCounter("hello great world") === LineCounter(true, 1, true))
    }

    it("should count words in the middle and add not finished marks at the start") {
      assert(WordsCounter.lineWordsCounter("hello great world.") === LineCounter(true, 2, false))
    }

    it("should count words in the middle and add not finished marks at the end") {
      assert(WordsCounter.lineWordsCounter(" hello great world") === LineCounter(false, 2, true))
    }

    it("should count all words when they are surrounded with punctuation") {
      assert(WordsCounter.lineWordsCounter(" hello great world ") === LineCounter(false, 3, false))
    }
    ...
    describe("punctuation marks - beginning") {
      ...
      it("should cover all punctuations mark in the beginning") {
        allSeparators.foreach(mark => {
          assert(WordsCounter.lineWordsCounter(mark + "hello great world") === LineCounter(false, 2, true))
        })
      }
    }
    ...
    describe("punctuation marks - ending") {
      ...
      it("should cover all punctuations mark in the ending") {
        allSeparators.foreach(mark => {
          assert(WordsCounter.lineWordsCounter("hello great world" + mark) === LineCounter(true, 2, false))
        })
      }
    }
  }
  ...
  describe("monoid") {
    ...
    describe("unfinished words between lines") {
      it("should sum together two lines without not finished words") {
        assert(monoid.op(LineCounter(false, 3, false), LineCounter(false, 5, false)) === LineCounter(false, 8, false))
      }

      it("should sum together two lines when left has potentially unfinished word") {
        assert(monoid.op(LineCounter(false, 3, true), LineCounter(false, 5, false)) === LineCounter(false, 9, false))
      }

      it("should sum together two lines when right has potentially unfinished word") {
        assert(monoid.op(LineCounter(false, 3, false), LineCounter(true, 5, false)) === LineCounter(false, 9, false))
      }

      it("should sum together two lines when both have unfinished word in the middle") {
        assert(monoid.op(LineCounter(false, 3, true), LineCounter(true, 5, false)) === LineCounter(false, 9, false))
      }
    }

    describe("unfinished on the sides") {
      it("should sum together two lines with unfinished on left side") {
        assert(monoid.op(LineCounter(true, 3, false), LineCounter(false, 5, false)) === LineCounter(true, 8, false))
      }

      it("should sum together two lines with unfinished on right side") {
        assert(monoid.op(LineCounter(false, 3, false), LineCounter(false, 5, true)) === LineCounter(false, 8, true))
      }

      it("should sum together two lines with unfinished on right side and middle") {
        assert(monoid.op(LineCounter(false, 3, true), LineCounter(false, 5, true)) === LineCounter(false, 9, true))
      }

      it("should sum together two lines with unfinished everywhere") {
        assert(monoid.op(LineCounter(true, 3, true), LineCounter(true, 5, true)) === LineCounter(true, 9, true))
      }
    }
  }
